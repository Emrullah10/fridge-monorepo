...
Eğer proje tek bir backend servisiyle başlıyorsa (mikroservis mimarisi gerekmiyorsa):
...
Yeni bir projede önce minimum kurulumla başlamak, hook script'lerinin (özellikle `post-write.js`'teki `detectFixPattern` sezgiselleri) zaman içinde projeye özel ihtiyaçlara göre ayarlanmasını beklemek makul bir yol olabilir; hook'lar jenerik oldukları için doğrudan taşınabilir, ama incelikleri (örn. hangi dosya uzantılarının "code" sayılacağı) hedef projenin diline göre gözden geçirilmelidir.
